...
$pass = $_POST['sno'];
$deg = $_POST['desg'];
$id=GenerateId("teacher",1);

$sql="insert into teacher values('$name','$email','$fno','$mobile','$address','$deg','$id')";
$sql1="insert into logindata1 values('$email','$pass','teacher')";
...
$slot = $_POST['slot'];
$id=GenerateId("subjectt",1);

if (!empty($name) || !empty($subjectno) || !empty($maxno) || !empty($sem) || !empty($branch) || !empty($session) || !empty($type) ) {
    $sql = "INSERT INTO subjectt 
	values ( '$id','$name', '$branch','$sem', '$session','$subjectno' ,'$slot', '$maxno','$type')";
	if ($cn->query($sql)){
